...
with (ROOT/(name+'-gpu.csv')).open('w') as out:
    gpu=subprocess.Popen(['nvidia-smi','--query-gpu=timestamp,name,driver_version,memory.used,utilization.gpu,temperature.gpu,power.draw,clocks.current.graphics,clocks.current.memory,fan.speed,clocks_event_reasons.sw_thermal_slowdown,clocks_event_reasons_counters.sw_thermal_slowdown','--format=csv','--loop-ms=1000'],stdout=out,stderr=subprocess.DEVNULL,creationflags=0x08000000)
