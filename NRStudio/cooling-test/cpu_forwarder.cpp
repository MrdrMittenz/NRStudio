...
#include <cstdarg>
#include <cstring>
static LONG native_frames = 0;
static void native_log(const char* format, ...) {
    HMODULE self = nullptr;
    GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                      reinterpret_cast<const char*>(&native_log), &self);
    char path[MAX_PATH] = {};
    if (!GetModuleFileNameA(self, path, MAX_PATH)) return;
    char* slash = strrchr(path, '\\'); if (!slash) return;
    strcpy_s(slash + 1, MAX_PATH - (slash + 1 - path), "dlssnr-native.log");
    FILE* file = nullptr; fopen_s(&file, path, "a"); if (!file) return;
    fprintf(file, "pid=%lu tick=%llu ", GetCurrentProcessId(), GetTickCount64());
...
__declspec(dllexport) void *dlssnr_call_create(const wchar_t *snippetPath, const wchar_t *dataPath,
    ...
    native_log("create init=%08x result=%08x size=%ux%u float_slot=%d feature=%p", dlssnr_call_last_init, dlssnr_call_last_create, width, height, g_floatSlot, handle);
    return handle;
}
...
__declspec(dllexport) int dlssnr_call_evaluate(ID3D12GraphicsCommandList *cmd, void *feature,
    ...
    LARGE_INTEGER cpuStart{},cpuEnd{},cpuFrequency{};
    QueryPerformanceFrequency(&cpuFrequency);QueryPerformanceCounter(&cpuStart);
    volatile int result = g_snip.evaluate(cmd, feature, capabilityParams, nullptr);
    QueryPerformanceCounter(&cpuEnd);
    native_log("cpu-evaluate-ms=%.6f",double(cpuEnd.QuadPart-cpuStart.QuadPart)*1000.0/double(cpuFrequency.QuadPart));
