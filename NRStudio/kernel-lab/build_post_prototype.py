...
This does not patch the model. Helpers deliberately retain explicit calls for
initial validation; this is not a performance-optimized implementation.
"""
import hashlib
import json
import re
...
helpers = (root / 'post_helpers.ptx').read_text()
...
(root / 'post-prototype-build.json').write_text(json.dumps(dict(
    ...
    limitations='Compatibility prototype with helper calls; not deployed or optimized.'), indent=2))
