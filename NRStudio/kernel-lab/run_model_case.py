...
p=argparse.ArgumentParser();p.add_argument('case');p.add_argument('--kernel',choices=['swin1','swin8','pre','post']);p.add_argument('--cubin');p.add_argument('--prepared',action='store_true');p.add_argument('--alternate',action='store_true');p.add_argument('--width',type=int,default=256);p.add_argument('--height',type=int,default=256);p.add_argument('--frames',type=int,default=10);args=p.parse_args()
...
env=os.environ.copy()
for key in ['NRSTUDIO_TEST_CUBIN','NRSTUDIO_TEST_KERNEL','NRSTUDIO_PREP_CUBIN','NRSTUDIO_PREPARED_ALTERNATE','NRSTUDIO_CONTROL_CUBIN']:env.pop(key,None)
if args.alternate:
 assert args.prepared
 env['NRSTUDIO_PREPARED_ALTERNATE']='1'
 env['NRSTUDIO_CONTROL_CUBIN']=str(root/'post-word-raw.cubin')
if args.prepared:
 assert args.kernel=='post' and args.cubin
 env['NRSTUDIO_PREP_CUBIN']=str(root/'prepare_weights.cubin')
if args.cubin:
