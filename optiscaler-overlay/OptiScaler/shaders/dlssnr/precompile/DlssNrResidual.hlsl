...
Texture2D<float4>   gResidual : register(t4);  // signed edit in normalised linear light (HDR), or game SDR units
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    if (gMode == 2)
    {
        if (gComposeMode == 1)
        {
            // Integrate each source pixel's overlap with this output pixel. Bilinear sampling alone
            // aliases thin lines at fractional working scales. This does not change the original.
            uint sw, sh;
            gSource.GetDimensions(sw, sh);
            if (sw == gWidth && sh == gHeight)
            {
                // No filter is needed, and a decode/encode round trip would add rounding noise.
                gTarget[id.xy] = gSource.Load(int3(id.xy, 0));
                return;
            }
            float2 begin = float2(id.xy) * float2(sw, sh) / float2(gWidth, gHeight);
            float2 end = float2(id.xy + 1) * float2(sw, sh) / float2(gWidth, gHeight);
            float4 sum = 0;
            for (int y = (int)floor(begin.y); y < (int)ceil(end.y); ++y)
                for (int x = (int)floor(begin.x); x < (int)ceil(end.x); ++x)
                {
                    float2 overlap = max(0.0, min(end, float2(x + 1, y + 1)) - max(begin, float2(x, y)));
                    float4 sample = gSource.Load(int3(clamp(int2(x, y), int2(0, 0), int2(sw, sh) - 1), 0));
                    // HDR encode produces an sRGB proxy. Average its represented light, not its
                    // nonlinear code values: a half-covered white light should retain half its
                    // proxy energy. Passthrough buffers have no transfer function supplied here.
                    if (gPassthrough == 0) sample.rgb = SrgbToLinear(sample.rgb);
                    sum += sample * overlap.x * overlap.y;
                }
            float4 filtered = sum / max((end.x - begin.x) * (end.y - begin.y), 1e-6);
            if (gPassthrough == 0) filtered.rgb = LinearToSrgb(filtered.rgb);
            gTarget[id.xy] = filtered;
            return;
        }
    ...
    if (gMode == 3)
    {
        // Decode matched texels BEFORE subtracting and interpolating. Decoding a bilinearly sampled
        // sRGB answer and input separately is a different operation, especially across hard edges.
        ...
        gTarget[id.xy] = float4(clamp(delta, -65504.0, 65504.0), 0);
        return;
    }
    ...
    if (gMode == 0)
    {
        ...
        gKeep[id.xy] = gComposeMode == 1 ? source : float4(frame, source.a);
    ...
    float3 edit = gComposeMode == 1 ? gResidual.SampleLevel(gLinear, cmpUv, 0).rgb : model - proxy;
    ...
    if (gComposeMode == 1)
    {
        float3 delta = gResidual.SampleLevel(gLinear, cmpUv, 0).rgb;
        float3 controlled = delta * gResidualDetail;
        if (gResidualTone != gResidualDetail)
        {
            uint rw, rh;
            gResidual.GetDimensions(rw, rh);
            float3 low = 0;
            [unroll] for (int y = -1; y <= 1; ++y)
                [unroll] for (int x = -1; x <= 1; ++x)
                    low += gResidual.SampleLevel(gLinear, cmpUv + float2(x, y) / float2(rw, rh), 0).rgb
                        * ((x == 0 ? 2.0 : 1.0) * (y == 0 ? 2.0 : 1.0) / 16.0);
            controlled += low * (gResidualTone - gResidualDetail);
        }
        controlled *= gTransferStrength;

        // The identity route avoids even a divide/multiply of the original HDR value. Colour at
        // zero keeps the game's chromaticity; the guard applies to both colour and luminance edits.
        float3 result = originalSample.rgb;
        if (any(controlled != 0) && all(isfinite(controlled)))
        {
            const float floorLuma = 1.0 / 512.0;
            float ratio = clamp((originalLuma + dot(controlled, kLuma) + floorLuma)
                / max(originalLuma + floorLuma, floorLuma), 0.0, gMaxRatio);
            result = lerp(originalSample.rgb * ratio, originalSample.rgb + controlled * normScale, gColourStrength);
            float ceiling = max(originalLuma, 0.0) * gMaxRatio + floorLuma * max(0.0, gMaxRatio - 1.0);
            float resultLuma = dot(max(result, 0.0), kLuma) / normScale;
            if (resultLuma > ceiling) result *= ceiling / max(resultLuma, floorLuma);
            result = max(result, 0.0);
            if (!all(isfinite(result))) result = originalSample.rgb;
        }
        if (showOriginal) result = originalSample.rgb;
        if (outsideFrame) result = 0;
        if (onDivider) result = gWhitePoint;
